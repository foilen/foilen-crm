...
public class ItemServiceImplTest extends AbstractSpringTests {
    ...
    class BillPendingTests {
        ...
        void billPendingWithNeverUsedPrefix() {
            List<?> initialTransactions = trimTransaction(transactionRepository.findAll(Sort.by("invoiceId")));
            List<?> initialItems = trimItem(itemRepository.findAll(Sort.by("invoiceId", "description")));
        ...
        void billSomePendingWithNeverUsedPrefix() {
            ...
            form.getItemToBillIds().add(itemRepository.findByClientIdAndInvoiceIdNullAndDescription(clientRepository.findByShortName("avez").getId(), "Shared hosting - L1").getId());
            form.getItemToBillIds().add(itemRepository.findByClientIdAndInvoiceIdNullAndDescription(clientRepository.findByShortName("zooa").getId(), "Shared hosting - L2").getId());
    ...
    class DeleteItemTests {
        ...
        void deleteNotPendingFails() {
            ...
            String itemId = itemRepository.findAllByInvoiceIdNotNull(PageRequest.of(0, 1)).getContent().getFirst().getId();
